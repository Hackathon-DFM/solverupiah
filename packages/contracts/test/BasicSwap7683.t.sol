...
contract BasicSwap7683ForTest is BasicSwap7683 {
  ...
  ) public view returns (ResolvedCrossChainOrder memory rOrder) {
    (rOrder, , ) = _resolvedOrder(
      _orderType,
      _sender,
      _openDeadline,
      _fillDeadline,
  ...
  function setOrderOpened(bytes32 _orderId, OrderData memory orderData) public {
    openOrders[_orderId] = abi.encode(
...
contract BasicSwap7683Test is BaseTest {
  ...
  bytes32 internal wrongMsgSender =
    makeAddr("wrongMsgSender").addressToBytes32();
  ...
  function test__settleOrders_works() public {
    ...
    assertEq(
      baseSwap.dispatchedOrdersFillerData(0),
      abi.encode("some filler data1")
    );
    assertEq(
      baseSwap.dispatchedOrdersFillerData(1),
      abi.encode("some filler data2")
    );
  }
  ...
  function test__handleSettleOrder_works() public {
    ...
    assertEq(
      balancesAfter[balanceId[address(baseSwap)]],
      balancesBefore[balanceId[address(baseSwap)]] - amount
    );
    assertEq(
      balancesAfter[balanceId[karpincho]],
      balancesBefore[balanceId[karpincho]] + amount
    );
  }
  ...
  function test__handleSettleOrder_not_OPENED() public {
    ...
    assertEq(
      balancesAfter[balanceId[address(baseSwap)]],
      balancesBefore[balanceId[address(baseSwap)]]
    );
    assertEq(
      balancesAfter[balanceId[karpincho]],
      balancesBefore[balanceId[karpincho]]
    );
  }
  ...
  function test__handleRefundOrder_works() public {
    ...
    assertEq(
      balancesAfter[balanceId[kakaroto]],
      balancesBefore[balanceId[kakaroto]] + amount
    );
  }
  ...
  function test__resolveOrder_gasless_works() public view {
    ...
    (ResolvedCrossChainOrder memory rOrder, , ) = baseSwap.resolveOrder(
      order,
      new bytes(0)
    );
  ...
  function test__resolveOrder_InvalidOrderType() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        BasicSwap7683.InvalidOrderType.selector,
        wrongOrderType
      )
    );
  ...
  function test__resolveOrder_InvalidOriginDomain() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        BasicSwap7683.InvalidOriginDomain.selector,
