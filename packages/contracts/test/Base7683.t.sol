...
contract Base7683Test is BaseTest {
  ...
  function test_openFor_OrderOpenExpired(
    uint32 _fillDeadline,
    uint32 _openDeadline
  ) public {
    vm.assume(_openDeadline < block.timestamp);
  ...
  function test_openFor_InvalidGaslessOrderSettler(
    uint32 _fillDeadline,
    uint32 _openDeadline
  ) public {
    vm.assume(_openDeadline > block.timestamp);
  ...
  function test_openFor_InvalidGaslessOrderOrigin(
    uint32 _fillDeadline,
    uint32 _openDeadline
  ) public {
    vm.assume(_openDeadline > block.timestamp);
  ...
  function test_openFor_InvalidNonce(
    uint32 _fillDeadline,
    uint32 _openDeadline
  ) public {
    vm.assume(_openDeadline > block.timestamp);
  ...
  // resolveFor
  function test_resolveFor_works(
    uint32 _fillDeadline,
    uint32 _openDeadline
  ) public {
    bytes memory orderData = abi.encode("some order data");
    ...
    vm.prank(karpincho);
    ResolvedCrossChainOrder memory resolvedOrder = base.resolveFor(
      order,
      new bytes(0)
    );
  ...
  function test_fill_works() public {
    ...
    (bytes memory _originData, bytes memory _fillerData) = base.filledOrders(
      orderId
    );
