...
contract BaseTest is Test, DeployPermit2 {
  ...
  function _getOrderIDFromLogs()
    internal
    returns (bytes32, ResolvedCrossChainOrder memory)
  {
  ...
  function _orderDataById(
    bytes32 orderId
  ) internal view returns (bytes memory orderData) {
  ...
  function _assertOrder(
    ...
    assertEq(
      balancesBefore[balanceId[sender]] - amount,
      balancesAfter[balanceId[sender]]
    );
    assertEq(
      balancesBefore[balanceId[receiver]] + amount,
      balancesAfter[balanceId[receiver]]
    );
  }
}
