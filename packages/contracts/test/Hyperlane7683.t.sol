...
contract Hyperlane7683ForTest is Hyperlane7683 {
  ...
  constructor(
    address _mailbox,
    address permitt2
  ) Hyperlane7683(_mailbox, permitt2) {}
...
contract Hyperlane7683Test is BaseTest {
  ...
  function _deployProxiedRouter(
    MockMailbox _mailbox,
    address _owner
  ) internal returns (Hyperlane7683ForTest) {
    Hyperlane7683ForTest implementation = new Hyperlane7683ForTest(
      address(_mailbox),
      permit2
    );
  ...
  function setUp() public override {
    ...
    destinationRouter = _deployProxiedRouter(
    ...
    destinationRouterB32 = TypeCasts.addressToBytes32(
  ...
  function test__dispatchSettle_works() public enrollRouters {
      ...
      abi.encodeCall(
        ...
        (
          ...
          StandardHookMetadata.formatMetadata(
            uint256(0),
            originRouter.destinationGas(destination),
            kakaroto,
            ""
          ),
    ...
    vm.prank(kakaroto);
    originRouter.dispatchSettle{value: gasPaymentQuote}(
      destination,
      orderIds,
      ordersFillerData
    );
  ...
  function test__handle_settle_works() public enrollRouters {
    ...
    deal(kakaroto, 1_000_000);
    vm.prank(kakaroto);
    destinationRouter.dispatchSettle{value: gasPaymentQuote}(
      origin,
      orderIds,
      ordersFillerData
    );
    ...
    assertEq(
      originRouter.settledMessageSender(0),
      TypeCasts.addressToBytes32(address(destinationRouter))
    );
    assertEq(
      originRouter.settledMessageSender(1),
      TypeCasts.addressToBytes32(address(destinationRouter))
    );
    ...
    assertEq(
      originRouter.settledOrderReceiver(0),
      TypeCasts.addressToBytes32(receiver1)
    );
    assertEq(
      originRouter.settledOrderReceiver(1),
      TypeCasts.addressToBytes32(receiver2)
    );
  }
